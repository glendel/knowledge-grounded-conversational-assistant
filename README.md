...
The Core now also provides durable, local, scoped chat continuity. It persists only bounded and redacted recent turns plus explicitly safe chat facts, expires it under deployment-configured retention, and never treats it as approved Knowledge Base evidence. An unreadable or unwritable memory record is observed without replacing natural model-led conversation. The conversation intelligence policy can expand retrieval from relevant recent user goals, bring in explicitly related approved knowledge, supply bounded non-factual continuity, and optionally ask the configured model for one hidden final quality pass. It never turns raw sources, memory, user corrections, or model drafts into factual evidence.
