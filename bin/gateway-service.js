...
export async function runGatewayService(argumentsList, { environment = process.env, stdout = process.stdout } = {}) {
  ...
  if (options.processOnce) return runGatewayWork(runtime);
  ...
  return Object.freeze({ runtime, service, address, stop: async () => { clearInterval(timer); await service.close(); } });
}

export function parseOptions(values) {
  const options = { help: false, deploymentRoot: null, host: '127.0.0.1', port: 3000, processOnce: false };
...
function helpText() { return 'Usage: node --env-file-if-exists=<deployment .env> ./bin/gateway-service.js --deployment-root <absolute-path> [--host 127.0.0.1] [--port 3000] [--process-once]\n'; }
