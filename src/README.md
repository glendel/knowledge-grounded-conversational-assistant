...
`src/conversation/` provides approved-only retrieval, bounded context assembly, actual-text language selection, continuity-aware retrieval, and a model-led prose-generation loop. Focused curated knowledge is preferred over large development-coverage records, while coverage remains available as a fallback when it is the only approved evidence. A deployment may opt into one hidden final quality pass; the reviewer may improve naturalness and directness but cannot make memory, user text, or a draft into approved business evidence. This module must not read raw knowledge, discover deployment roots, invoke a dialogue tree, or demand JSON from normal user-facing prose.
